...
public enum TableCreationQuery {
    ...
    CREATE_SCHEDULE_TABLE("CREATE TABLE IF NOT EXISTS SCHEDULE (schedule_id SERIAL PRIMARY KEY, bus_id BIGINT, driver_id BIGINT, route_id BIGINT, starting_time TIME NOT NULL, ending_time TIME NOT NULL, total_travel_time_min INT, total_distance_km DECIMAL(10, 2), rest_time_minutes DECIMAL(10 , 2), day VARCHAR(10), FOREIGN KEY (bus_id) REFERENCES Bus(busId), FOREIGN KEY (driver_id) REFERENCES Driver(driverId), FOREIGN KEY (route_id) REFERENCES Route(routeId));");
